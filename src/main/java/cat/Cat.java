...
import cat.option.OptionHandler;
import cat.exception.CancelException;
import cat.exception.SkipFileException;
...
public class Cat {
    ...
    public boolean concatenate(String... args) {
        try {
            final OptionHandler handler = new OptionHandler(args);
    ...
    private InputStreamReader getInputStreamReader(String arg)
        ...
        if (Objects.isNull(arg) || arg.isBlank() || Const.STD_INPUT_PARAM.equals(arg)) {
            return getStandardInputStreamReader();
        } else {
            return getFileInputStreamReader(arg);
        }
    }
     ...
     * @throws SkipFileException ファイル読込みが失敗した場合
     */
    private InputStreamReader getFileInputStreamReader(String filePath)
            throws SkipFileException {
        return getFileInputStreamReader(filePath, StandardCharsets.UTF_8.name());
    }
     ...
     * @throws SkipFileException ファイル読込みが失敗した場合
     */
    InputStreamReader getFileInputStreamReader(String filePath, String encoding) throws SkipFileException {
     ...
     */
    private InputStreamReader getStandardInputStreamReader() throws SkipFileException {
